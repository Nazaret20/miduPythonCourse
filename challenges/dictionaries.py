...
def recuento_votos(votos):
    ...
    for persona in recuento_final:
        if recuento_final[persona] > mas_votos:
            mas_votos = recuento_final[persona]
            nombre = persona

    candidatos_y_votos = ", ".join(f"{key} con {value}" for key, value in recuento_final.items())

    return f"Los candidatos con sus respectivos votos son: {candidatos_y_votos}, con lo cual el ganador es {nombre} con {mas_votos} votos."
